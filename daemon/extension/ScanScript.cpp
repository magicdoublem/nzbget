...
void ScanScriptController::AddMessage(Message::EKind kind, const char* text)
{
	...
	if (!strncmp(msgText, "[NZB] ", 6))
	{
		...
		{
			*m_priority = Util::StrToNum<int64>(msgText + 6 +9).value_or(0);
		}
		else if (!strncmp(msgText + 6, "TOP=", 4))
		{
			*m_addTop = Util::StrToNum<int>(msgText + 6 + 4).value_or(0) != 0;
		}
		else if (!strncmp(msgText + 6, "PAUSED=", 7))
		{
			*m_addPaused = Util::StrToNum<int>(msgText + 6 + 7).value_or(0) != 0;
		}
		...
		{
			*m_dupeScore = Util::StrToNum<int64>(msgText + 6 + 10).value_or(0);
		}
